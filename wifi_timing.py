...
def preprocessor(file_name):
    ...
    for pkt in cap:
        ...
        prev_pkt_time, macs, counter = mac_track(pkt, tgt_mac_track_file, prev_pkt_time, macs, counter)
...
def mac_track(pkt, tgt_mac_track_file, prev_pkt_time, macs, counter):
    """
    ...
    diff = float(pkt_time) - float(prev_pkt_time)

    # check if two consecutive packets are only 5 seconds apart
    # to ensure corrupt packets do not provide invalid time
    if float(pkt_time) < 0 or diff > 5:
        counter +=1
        ...
        # if five packets in a row are off then probably
        # a jump in timing, so start with new time
        if counter > 5:
            counter = 0
            return pkt_time, macs, counter
        else:
            return prev_pkt_time, macs, counter
    else:
        counter = 0
        ...
        if pkt_src in WIFI_DEVICES:
            ...
            for k, v in macs.items():
                if (float(pkt_time) - float(v[1])) > MAC_TRACK_TIME:
                    file_input = [DEVICE_NAME[k], helpers.pretty_time(v[0]), helpers.pretty_time(v[1])]
...
def mac_track_final(tgt_mac_track_file, pkt_time, macs):
    ...
    """
    for k, v in macs.items():
        file_input = [DEVICE_NAME[k], helpers.pretty_time(v[0]), helpers.pretty_time(pkt_time)]
        csv.writer(tgt_mac_track_file).writerow(file_input)
        del macs[k]
    ...
    for filename in os.listdir(DST_DIR):
        ...
        if device in IOT_DEVICES:
            ...
            for line in contents:
                ...
                pkt_src = line[2]
                pkt_dst = line[3]
                if pkt_src == RASPI or pkt_src == ROUTER:
                    device_by_dst.append([pkt_time, pkt_size, pkt_src, pkt_dst])
