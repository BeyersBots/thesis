...
def classifier(file_name):
    ...
    btle_output_file = "ble_events_" + DATE + ".csv"
    helpers.delete_file(btle_output_file)
    btle_device_file = "ble_devices_" + DATE + ".csv"
    ...
    total_time = final_time - total_time_start
    normalized_total_time = (TIMING_PKT_NUMBER * total_time) / pkt_cntr

    print "total number of packets: ", pkt_cntr
    print "Total time: ", total_time
    print "Normalized total time per 25000 packets: ", normalized_total_time

    csv.writer(pt_file).writerow(["BLE", pkt_cntr, total_time, normalized_total_time])
...
def ble_adv_ind(pkt, devices):
    ...
    if adv_addr not in devices:
        ...
        if (len(device_name) > 2) and (device_name in BLE_DEVICES):
...
def ble_connect_req(pkt, devices, connections):
    ...
    pkt_time = helpers.pretty_time(pkt_time)
    ...
    if (pkt_slv_id in BLE_DEVICES) and (fields not in connections):
