...
import sys, getopt, csv, os, datetime, time
...
WIFI_DEVICES = {'b8:27:eb:09:1a:81', '14:91:82:24:dd:35', '60:38:e0:ee:7c:e5', 'a0:18:28:33:34:f8', '08:66:98:ed:1e:19',
                ...
                'ec:1a:59:e4:fd:41', 'ec:4f:82:73:d1:1a', 'ec:4f:82:73:d1:1c'}
...
def mac_track(pkt, tgt_mac_track_file, prev_pkt_time, macs):
    ...
    try:
        ...
        if pkt_src in WIFI_DEVICES:
            ...
            if float(pkt_time) - float(prev_pkt_time) < 2:
                if pkt_src not in macs:
                    macs[pkt_src] = [pkt_time, pkt_time]
                else:
                    macs[pkt_src][1] = pkt_time

                for k, v in macs.items():
                    if (float(pkt_time) - float(v[1])) > 300:
                        file_input = [k, helpers.pretty_time(v[0]), helpers.pretty_time(v[1])]
                        csv.writer(tgt_mac_track_file).writerow(file_input)
                        del macs[k]
            else:
                # provide error information to assist in troubleshooting
                print "packet time error: ", pkt.number
                print "last valid time: ", prev_pkt_time
                print "current pckt time: ", pkt_time
...
def classifier():
    ...
    """
    # initialize file names
    dev_cat_file = "wifi_devices.csv"
    helpers.delete_file(dev_cat_file)
    event_id_file = "wifi_events.csv"
    helpers.delete_file(event_id_file)
    network_edge_file = "network_edge.csv"
    ...
    device_categorization, event_identification = events_by_dst()

    # identify events by destination
    event_identification = events_by_src(network_edge_file, device_categorization, event_identification)
...
def events_by_dst():
    ...
    return device_categorization, event_identification


def events_by_src(network_edge_file, device_categorization, event_identification):
    """
